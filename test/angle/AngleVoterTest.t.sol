...
contract AngleVoterTest is Test {
    ...
    function _castVote(uint8 _support, string memory _reason, bytes memory _params) internal {
        ...
        if (keccak256(abi.encode(_reason)) == keccak256(abi.encode(""))) {
            newVoter.castVote(proposalId, _support);
        } else if (_params.length == 0){
            newVoter.castVoteWithReason(proposalId, _support, _reason);
        } else {
            newVoter.castVoteWithReasonAndParams(proposalId, _support, _reason, _params);
        }
        
        assertTrue(angleGovernor.hasVoted(proposalId, angleLocker));

        (uint256 againstVotesAfterCast, uint256 forVotesAfterCast, uint256 abstainVotesAfterCast) = angleGovernor.proposalVotes(proposalId);
