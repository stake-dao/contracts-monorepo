...
import {ISdCakePositionNFT} from "src/base/interfaces/ISdCakePositionNFT.sol";
...
contract CakeStrategyNFTMinting is UUPSUpgradeable {
    ...
    /// @notice sdCakePositionNFT
    ISdCakePositionNFT public sdCpNft;
    ...
    modifier onlyNftStaker(uint256 tokenId) {
        if (msg.sender != sdCpNft.ownerOf(tokenId)) revert NotNftStaker();
        _;
    }
    ...
    function initialize(address _governance, address _executor, address _sdCpNft) external {
        if (governance != address(0)) revert Governance();
        governance = _governance;
        executor = IExecutor(_executor);
        sdCpNft = ISdCakePositionNFT(_sdCpNft);
    ...
    function harvestNftReward(uint256 _tokenId) external {
        address recipient =
            rewardRecipients[_tokenId] == address(0) ? sdCpNft.ownerOf(_tokenId) : rewardRecipients[_tokenId];
    ...
    function harvestNftsReward(uint256[] memory _tokenIds) external {
        ...
        for (uint256 i; i < tokensLength;) {
            recipient = rewardRecipients[_tokenIds[i]] == address(0)
                ? sdCpNft.ownerOf(_tokenIds[i])
                : rewardRecipients[_tokenIds[i]];
    ...
    function harvestNftFees(uint256 _tokenId) external {
        address recipient = feeRecipients[_tokenId] == address(0) ? sdCpNft.ownerOf(_tokenId) : feeRecipients[_tokenId];
    ...
    function harvestNftsFees(uint256[] memory _tokenIds) external {
        ...
        for (uint256 i; i < tokensLength;) {
            recipient =
                feeRecipients[_tokenIds[i]] == address(0) ? sdCpNft.ownerOf(_tokenIds[i]) : feeRecipients[_tokenIds[i]];
    ...
    function harvestNftAll(uint256 _tokenId) external {
        ...
        address feeRecipient =
            feeRecipients[_tokenId] == address(0) ? sdCpNft.ownerOf(_tokenId) : feeRecipients[_tokenId];
    ...
    function harvestNftsAll(uint256[] memory _tokenIds) external {
        ...
        for (uint256 i; i < tokenLength;) {
            rewardRecipient = rewardRecipients[_tokenIds[i]] == address(0)
                ? sdCpNft.ownerOf(_tokenIds[i])
                : rewardRecipients[_tokenIds[i]];
            feeRecipient =
                feeRecipients[_tokenIds[i]] == address(0) ? sdCpNft.ownerOf(_tokenIds[i]) : feeRecipients[_tokenIds[i]];
    ...
    function onERC721Received(address, address _from, uint256 _tokenId, bytes calldata) external returns (bytes4) {
        ...
        ISdCakePositionNFT(sdCpNft).mint(_from, _tokenId);
    ...
    function _withdrawNft(uint256 _tokenId, address _recipient) internal onlyNftStaker(_tokenId) {
        // burn the sdCakePositionNFT
        ISdCakePositionNFT(sdCpNft).burn(_tokenId);
    ...
    /// @notice Set sdCakePosition NFT
    /// @param _sdCpNft sdCpNft address.
    function setSdCpNft(address _sdCpNft) external onlyGovernance {
        sdCpNft = ISdCakePositionNFT(_sdCpNft);
    }
