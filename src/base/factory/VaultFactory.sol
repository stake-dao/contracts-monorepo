...
import "src/base/interfaces/IStrategyVault.sol";
...
abstract contract VaultFactory {
    ...
    constructor(address _strategy, address _sdtDistributor, address _vaultImpl) {
    ...
    function cloneAndInit(address _gauge) public virtual {
        ...
        // deploy gauge
        ILiquidityGaugeStrat sdGauge = _cloneAndInitGauge(GAUGE_IMPL, address(vault), GOVERNANCE, tokenName);
     ...
     */
    function _cloneVault(address _impl, address _lpToken, bytes32 _paramsHash) internal returns (IStrategyVault) {
        address deployed = address(_impl).cloneDeterministic(keccak256(abi.encodePacked(_lpToken, _paramsHash)));
