...
contract AccumulatorMigrationSafeModule is SafeModule, Ownable2Step {
    ///////////////////////////////////////////////////////////////
    /// --- IMMUTABLES
    ///////////////////////////////////////////////////////////////
    ...
    /// @notice Throws if the old accumulator is not enabled or is the address as the migrator.
    error InvalidOldAccumulator();

    /// @notice Throws if the new accumulator is address 0 or is the address as the migrator.
    error InvalidNewAccumulator();

    /// @notice Throws if the locker is not the same as the old and new accumulator.
    ...
    /// @notice Claims rewards on the `oldAccumulator` and immediately performs
    ///         the migration to `newAccumulator`.
    /// @param oldAccumulator Address of the accumulator being replaced.
    /// @param newAccumulator Address of the accumulator that will take over.
    /// @param components     List of contracts that must be updated to point to
    ///                       the new accumulator via `setAccumulator(address)`.
    function claimAndMigrate(address oldAccumulator, address newAccumulator, address[] calldata components) external {
        IAccumulator(oldAccumulator).claimAndNotifyAll();
        migrate(oldAccumulator, newAccumulator, components);
    }
    ...
    ///         3. Calls `setAccumulator(newAccumulator)` on every contract in
    ///            `components`.
    ...
    ///                       cases to migrate specific extra contracts like a strategy,
    ///                       a depositor, etc.
    function migrate(address oldAccumulator, address newAccumulator, address[] calldata components) public onlyOwner {
        ...
        address[] memory rewards = IAccumulator(newAccumulator).getRewards();
        for (uint256 i; i < rewards.length; i++) {
            gauge.set_reward_distributor(rewards[i], newAccumulator);
        }

        // Set the new accumulator in the extra relevant components
        for (uint256 i; i < components.length; i++) {
            if (address(components[i]) != address(0)) {
                _executeTransaction(
                    address(components[i]), abi.encodeWithSignature("setAccumulator(address)", newAccumulator)
                );
            }
        }
    ...
    /// @notice Returns the unique identifier of the contract.
    function identifier() external view virtual returns (string memory) {
        return type(AccumulatorMigrationSafeModule).name;
    }
}
