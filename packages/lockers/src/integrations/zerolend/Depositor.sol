...
import {DepositorBase, ITokenMinter, ILiquidityGauge} from "src/DepositorBase.sol";
...
contract Depositor is DepositorBase {
    ...
    function _createLock(uint256 _amount) internal returns (uint256 _tokenId) {
        (bool _success, bytes memory _data) = ISafeLocker(locker).execTransactionFromModuleReturnData(
            address(zeroLocker),
            0,
    ...
    function _merge(uint256 _tokenIdFrom, uint256 _tokenIdTo) internal {
        (bool _success,) = ISafeLocker(locker).execTransactionFromModuleReturnData(
            address(zeroLocker),
            0,
