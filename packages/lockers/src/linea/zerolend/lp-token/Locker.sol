...
contract Locker is VeCRVLocker {
    ...
    IZeroBaseLocker public immutable zeroLpLocker;

    uint256 public lockerTokenId;

    constructor(address _zeroLpLocker, address _governance, address _token, address _veToken)
        VeCRVLocker(_governance, _token, _veToken)
    {
        zeroLpLocker = IZeroBaseLocker(_zeroLpLocker);
    ...
    function createLock(uint256 _value, uint256 _unlockTime) external override onlyGovernanceOrDepositor {
        IERC20(token).safeApprove(address(zeroLpLocker), _value);

        // create the lock
        lockerTokenId = zeroLpLocker.createLock(_value, _unlockTime, true);
    ...
    function increaseLock(uint256 _value, uint256 _unlockTime) external override onlyGovernanceOrDepositor {
        ...
        if (_unlockTime > 0) {
            bool _canIncrease = (_unlockTime / 1 weeks * 1 weeks) > (zeroLpLocker.lockedEnd(lockerTokenId));
