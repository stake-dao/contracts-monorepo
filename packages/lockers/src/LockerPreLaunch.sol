...
/// @title LockerPreLaunch
/// @dev This contract implements a state machine with three states: IDLE, ACTIVE, and CANCELED
/**
 * @notice A contract that enables secure token locking before full protocol deployment. The LockerPreLaunch
...
contract LockerPreLaunch is IPreLaunchLocker {
    ///////////////////////////////////////////////////////////////
    ...
    /// @custom:slot 1
    DepositorPreLaunch public depositor;
    ...
    function lock(address _depositor) external onlyGovernance {
        ...
        if (DepositorPreLaunch(_depositor).token() != storedToken) revert INVALID_TOKEN();

        // ensure the given depositor has the same gauge as the one stored in this contract
        if (DepositorPreLaunch(_depositor).gauge() != address(gauge)) revert INVALID_GAUGE();

        // ensure the given depositor has the same sdToken as the one stored in this contract
        if (DepositorPreLaunch(_depositor).minter() != address(storedSdToken)) revert INVALID_SD_TOKEN();
