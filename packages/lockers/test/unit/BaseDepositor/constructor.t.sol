...
import {BaseDepositor} from "test/unit/BaseDepositor/utils/BaseDepositorTest.t.sol";
...
contract BaseDepositor__constructor is BaseTest {
    ...
    function test_RevertIfTheTokenIsTheZeroAddress() external {
        ...
        vm.expectRevert(BaseDepositorContract.ADDRESS_ZERO.selector);
        new BaseDepositor(address(0), makeAddr("locker"), makeAddr("minter"), makeAddr("gauge"), 100);
    }
    ...
    function test_RevertIfTheLockerIsTheZeroAddress() external {
        ...
        vm.expectRevert(BaseDepositorContract.ADDRESS_ZERO.selector);
        new BaseDepositor(makeAddr("token"), address(0), makeAddr("minter"), makeAddr("gauge"), 100);
    }
    ...
    function test_RevertIfTheMinterIsTheZeroAddress() external {
        ...
        vm.expectRevert(BaseDepositorContract.ADDRESS_ZERO.selector);
        new BaseDepositor(makeAddr("token"), makeAddr("locker"), address(0), makeAddr("gauge"), 100);
    }
    ...
    function test_RevertIfTheGaugeIsTheZeroAddress() external {
        ...
        vm.expectRevert(BaseDepositorContract.ADDRESS_ZERO.selector);
        new BaseDepositor(makeAddr("token"), makeAddr("locker"), makeAddr("minter"), address(0), 100);
    }
    ...
    function test_SetsTheGovernanceToTheCaller(address caller) external {
        ...
        vm.prank(caller);
        BaseDepositor depositor =
            new BaseDepositor(makeAddr("token"), makeAddr("locker"), minter, makeAddr("gauge"), 100);
    ...
    function test_SetsTheMaxLockDurationToTheGivenValue(uint256 maxLockDuration) external {
        ...
        BaseDepositor depositor =
            new BaseDepositor(makeAddr("token"), makeAddr("locker"), minter, makeAddr("gauge"), maxLockDuration);
    ...
    function test_SetsTheStateToACTIVE() external {
        ...
        assertEq(uint256(depositor.state()), uint256(BaseDepositorContract.STATE.ACTIVE));
    }
