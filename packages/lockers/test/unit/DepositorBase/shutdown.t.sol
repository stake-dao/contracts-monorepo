...
contract BaseDepositor__shutdown is BaseDepositorTest {
    function test_SetsTheStateToCANCELED() external {
        ...
        assertEq(uint256(baseDepositor.state()), uint256(DepositorBase.STATE.ACTIVE));

        vm.prank(governance);
        baseDepositor.shutdown();

        assertEq(uint256(baseDepositor.state()), uint256(DepositorBase.STATE.CANCELED));
    }
    ...
    function test_EmitsTheStateUpdatedEvent() external {
        ...
        emit StateUpdated(DepositorBase.STATE.CANCELED);
    ...
    function test_RevertsIfTheCallerIsNotTheGovernance() external {
        ...
        vm.expectRevert(DepositorBase.GOVERNANCE.selector);
        baseDepositor.shutdown();
    }
}
