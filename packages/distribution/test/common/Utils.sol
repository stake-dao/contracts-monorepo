...
contract Utils is Test {
    ...
    function generateMerkleProof(
        ...
        require(
            userAddresses.length == amounts.length,
        ...
        )
    {
        ...
        for (uint256 i = 0; i < users.length; i++) {
            ...
            // Get the amount
            string memory amountHex = vm.parseJsonString(
                json,
                string.concat("$.merkle.", users[i], ".amount.hex")
            );

            amounts[i] = vm.parseUint(amountHex);

            // Get the proof
            string[] memory proofStrings = vm.parseJsonStringArray(
                json,
                string.concat("$.merkle.", users[i], ".proof")
            );
    ...
    function getMerkleRootAndTotal()
        internal
        view
        returns (bytes32 merkleRoot, uint256 total)
    {
        string memory root = vm.projectRoot();
        string memory merklePath = string.concat(root, MERKLE_DATA_PATH);
