...
contract CurveFraxtalIntegrationTest is CurveIntegration {
    ...
    function _afterSetup() internal override {
        factory = address(
            new L2CurveFactory({
                childLiquidityGaugeFactory: CurveProtocol.FACTORY,
    ...
    function simulateRewards(RewardVault vault, uint256 amount) internal override {
        address locker = config.locker;
        if (locker == address(0)) locker = address(gateway);

        address gauge = vault.gauge();

        uint256 currentMinterBalance = _balanceOf(address(rewardToken), config.minter);

        // Make sure the minter has enough balance to mint the new amount
        deal(address(rewardToken), config.minter, currentMinterBalance + amount);
        ...
        try ILiquidityGauge(gauge).integrate_fraction(locker) returns (uint256 fraction) {
            ...
            currentIntegrateFraction = IMinter(config.minter).minted(locker, gauge);
        }
