...
contract PendleMainnetIntegrationTest is PendleIntegration {
    ...
    function simulateRewards(RewardVault vault, uint256 amount) internal override {
        address gauge = address(IPendleGauge(vault.gauge()));
    ...
    function _cheat_rewards(address gauge, address token, uint256 amount) internal {
        ...
        IPendleGauge.UserReward memory currentUserReward = IPendleGauge(gauge).userReward(token, _config.base.locker);
        uint256 userRewardSlot = stdstore.enable_packed_slots().target(gauge).sig("userReward(address,address)")
            .with_key(token).with_key(_config.base.locker).find();
        vm.store(
            gauge,
