...
import {IPendleMarket} from "src/base/interfaces/IPendleMarket.sol";
...
contract PendleStrategy {
    ...
    function withdraw(address _token, uint256 _amount, address _user) external {
        ...
        (bool success,) =
            ILocker(LOCKER).execute(_token, 0, abi.encodeWithSignature("transfer(address,uint256)", _user, _amount));
    ...
    function claim(address _token) external {
        ...
        // Distribute SDT
        ISdtDistributorV2(sdtDistributor).distribute(sdGauges[_token]);
    }
